...
const ensureDataDir = () => {
    ...
    try {
        ...
        if (!fs.existsSync(DATA_DIR)) {
            ...
            console.log(chalk.blue('[GIFT-X]📁 Created data directory'));
...
const loadDatabase = () => {
    ...
    try {
        ...
        console.log(chalk.blue('[GIFT-MD] 🔄 Using default database...'));
...
// Use correct property names
            global.botName = db.settings.botName;

            global.botOwner = db.settings.botOwner;

            global.owner = db.settings.ownerNumber;

            global.prefix = db.settings.prefix ;

            global.packname = db.settings.packname ;

            global.mode = db.settings.mode;

            global.version = db.settings.version;
        }
if (typeof global.initializeGlobals === 'function') {          global.initializeGlobals();
        }
    } catch (error) {
...
const initDatabase = () => {
    ...
    try {
        ...
        console.log(chalk.cyan('[GIFT-X] Connected to Database'));
